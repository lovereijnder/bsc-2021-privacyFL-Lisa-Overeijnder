...
from utils.dp_mechanisms import laplace, staircase, gaussian
...
def hgamma(epsilon):
    """Return heuristic gamma approximation."""
    return np.exp(epsilon) / 2
...
class ClientAgent(Agent):
    ...
    def initializations(self):
        """
        Preforms initializations that have to be done after initializing instance
        :return: None
        :rtype: None
        """
    ...
    def send_pubkeys(self):
        """
        Sends public keys to other clients in simulations as required by diffie-hellman protocol.
        """
    ...
    def initialize_common_keys(self):
        ...
        commonkeyList = dh.keyexchange(
            len(self.directory.clients),
            self.agent_number,
            pubkeyList,
            seckeyList,
            otherkeyList,
        )  # generates common keys
    ...
    def produce_weights(self, message):
        ...
        if iteration - 1 > len(self.train_datasets):  # iteration is indexed starting from 1
            raise (
                ValueError(
                    "Not enough data to support a {}th iteration. Either change iteration data length in config.py or "
                    "decrease amount of iterations.".format(iteration)
                )
            )
        ...
        final_weights, final_intercepts = copy.deepcopy(weights), copy.deepcopy(
            intercepts
        )

        if config.USE_DP_PRIVACY:
            if config.DP_ALGORITHM == 'Laplace' or config.DP_ALGORITHM == 'Gaussian' or config.DP_ALGORITHM == 'Staircase':
                lock.acquire()  # for random seed
                final_weights, final_intercepts = self.add_noise(
                    weights=weights, intercepts=intercepts, iteration=iteration
                )
                lock.release()
            else:
                raise AssertionError(
                    "Need to specify config.DP_ALGORITHM as Laplace, Gaussian or Staircase"
                )

        if config.USE_SECURITY:  # adding security via diffie-hellman offsets
            final_weights, final_intercepts = self.add_security_offsets(
        ...
        simulated_time += (
            computation_time + config.LATENCY_DICT[self.name]["server_agent0"]
        )
        ...
        return Message(
            ...
            recipient_name=self.directory.server_agents,
            body=body,
        )
    ...
    def add_noise(self, weights, intercepts, iteration):
        ...
        active_clients_lens = [
            config.LENS_PER_ITERATION[client_name]
        ...
        for i in range(weights_shape[0]):  # weights_modified is 2-D
            for j in range(weights_shape[1]):
                if config.DP_ALGORITHM == "Laplace":
                ...
                elif config.DP_ALGORITHM == "Staircase":
                    dp_noise = staircase(
                        mean=config.mean,
                        sensitivity=sensitivity,
                        epsilon=epsilon,
                        gamma=hgamma(epsilon),
                    )
                elif config.DP_ALGORITHM == "Gaussian":
                    dp_noise = gaussian(
                        mean=config.mean,
                        sensitivity=sensitivity,
                        epsilon=epsilon,
                    ...
                    raise AssertionError(
                        "Need to specify config.USE_DP_PRIVACY as Laplace, Gaussian or Staircase"
                    )
    ...
    def add_security_offsets(self, weights, intercepts):
        """
        Called if config.USE_SECURITY flag is on. Uses the offsets established by the diffie hellman key exchange
        ...
        for (
            agent_name,
            offset,
        ) in self.deltas.items():  # dictionary but should be ordered since Python 3
    ...
    def update_deltas(self):
        ...
        """
        if (
            None not in self.commonkeyList.values()
        ):  # if first time calling this function
            agents_and_seeds = self.commonkeyList.items()
            self.commonkeyList = self.commonkeyList.fromkeys(
                self.commonkeyList.keys(), None
            )
        else:
            # use existing seed to generate new seeds and offsets
    ...
    def receive_weights(self, message):
        ...
        """
        body = message.body
        iteration, return_weights, return_intercepts, simulated_time = (
            body["iteration"],
            body["return_weights"],
            body["return_intercepts"],
            body["simulated_time"],
        )
        ...
        converged = self.satisfactory_weights(
            (personal_weights, personal_intercepts), (return_weights, return_intercepts)
    ...
    def satisfactory_weights(self, personal, federated):
        ...
        return (weights_differences < config.tolerance).all() and (
            intercepts_differences < config.tolerance
        ).all()  # check all weights are close enough
