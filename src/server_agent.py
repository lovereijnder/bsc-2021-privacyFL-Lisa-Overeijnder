...
class ServerAgent(Agent):
    ...
    def request_values(self, num_iterations):
        """
        Method invoked to start simulation. Prints out what clients have converged on what iteration. Also prints out
        accuracy for each client on each iteration (what weights would be if not for the simulation) and federated
        accuracy.
        ...
        for i in range(1, num_iterations + 1):
            ...
            try:
                ...
                raise ValueError('''DATA INSUFFICIENT: Some client does not have a sample from each class so 
                dimension of weights is incorrect. Make train length per iteration larger for each client to avoid 
                this issue.''')
            ...
            for message in return_messages:
                if message.body['converged'] is True and message.sender not in converged:  # converging
