...
'''
This is a helper function for generating the probability density function for truncated laplace distribution.
...
def process(epsilon, global_sensitivity, A, B):
    lam = 1 / epsilon
    A_ = lam * np.log(2 + (1 - global_sensitivity) / global_sensitivity * np.exp(-B / lam) - 1 /
                      global_sensitivity * np.exp(-(B - 1) / lam))
    if abs(B) < abs(A_):
        return A_, B
    B_ = -lam * np.log(2 + (1 - global_sensitivity)/global_sensitivity * np.exp(A/lam) -
                       1/global_sensitivity * np.exp((B+1)/lam))
    if abs(A) < abs(B_):
        return A, B_
...
'''
Given privacy parameters epsilon, global_sensitivity, and scale parameters A, and B, 
return a function that is the probability density function for the truncated laplace distribution.
'''
...
# Example for getting results for truncated laplacian
